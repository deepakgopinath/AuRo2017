...
for kk=1:length(interfaces)
    ...
    for i=1:length(v1) %BLACK, WITHOUT, POUR
        if distri(uvals == v1(i)) > 1
            scatter(0.8, v1(i),'o','k', 'LineWidth', 1.5, 'jitter', 'on', 'jitterAmount', jit_factor(kk)*(distri(uvals == v1(i)) - 1));
        else
            scatter(0.8, v1(i),'o','k', 'LineWidth', 1.5);
        end
    end
    ...
    for i=1:length(v2) %RED, WITH, POUR
        if distri(uvals == v2(i)) > 1
            scatter(1.2, v2(i),'o','r', 'LineWidth', 1.5, 'jitter', 'on', 'jitterAmount', jit_factor(kk)*(distri(uvals == v2(i)) - 1));
            hold on;
        else
            scatter(1.2, v2(i),'o','r', 'LineWidth', 1.5);
        end
    end
    %G5 w wo plots
    uvals = unique(v3);
    ...
    for i=1:length(v3)%BLACK, WITHOUT, REACH
        if distri(uvals == v3(i)) > 1
            scatter(2.8, v3(i),'o','k', 'LineWidth', 1.5, 'jitter', 'on', 'jitterAmount', jit_factor(kk)*(distri(uvals == v3(i)) - 1));
        else
            scatter(2.8, v3(i),'o','k', 'LineWidth', 1.5);
        end
    end
    ...
    for i=1:length(v4)%RED, WITH, REACH
        if distri(uvals == v4(i)) > 1
            scatter(3.2, v4(i),'o','r', 'LineWidth', 1.5, 'jitter', 'on', 'jitterAmount', jit_factor(kk)*(distri(uvals == v4(i)) -  1));
            hold on;
        else
            scatter(3.2, v4(i),'o','r', 'LineWidth', 1.5);
        end
    end
...
%     
    if kk == 1
        set(gca, 'YTick', 0:5:ylims(kk));
        title('\bf \fontsize{16} Joystick  ');
        set(gca, 'XTick', 1:1:3);
    set(gca, 'XTickLabel', {'\bf \fontsize{10}Pout','', '\bf \fontsize{10}Reach'});
    ylabel('\fontsize{14} Number of Mode Switches');
    else
        set(gca, 'YTick', 0:10:ylims(kk));
         title('\bf \fontsize{16} Headarray');
         set(gca, 'XTick', 1:1:3);
    set(gca, 'XTickLabel', {'\bf \fontsize{10}Pour','', '\bf \fontsize{10}Reach'});
%     ylabel('\fontsize{14} Number of Mode Switches');
    end
   ...
   if kk ==1
    text(max(xlim)/2, max(ylim)-5, '\fontsize{11}\color{red}o - Disambiguation', 'HorizontalAlignment', 'Center','BackGroundColor','none', 'FontSize', 10);
    text(max(xlim)/2, max(ylim)-10, '\fontsize{11}\color{black}o - Manual', 'HorizontalAlignment', 'Center','BackGroundColor','none', 'FontSize', 10);
